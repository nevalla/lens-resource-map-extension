...
export class KubeResourceMap extends React.Component<{ id?: string }> {
  ...
  async componentDidMount(): Promise<void> {
    ...
    const deploymentStore: K8sApi.DeploymentStore = (K8sApi.apiManager.getStore(K8sApi.deploymentApi) as K8sApi.DeploymentStore);
    const statefulsetStore: K8sApi.StatefulSetStore = (K8sApi.apiManager.getStore(K8sApi.statefulSetApi) as K8sApi.StatefulSetStore);
    const daemonsetStore: K8sApi.DaemonSetStore = (K8sApi.apiManager.getStore(K8sApi.daemonSetApi) as K8sApi.DaemonSetStore);
    ...
    const podsStore: K8sApi.PodsStore = (K8sApi.apiManager.getStore(K8sApi.podsApi) as K8sApi.PodsStore);
    await Promise.all([deploymentStore.loadAll(), daemonsetStore.loadAll(),this.secretStore.loadAll(), this.serviceStore.loadAll(), podsStore.loadAll(), statefulsetStore.loadAll()])
    // Create chart
        ...
        const pods = podsStore.items.filter((item: K8sApi.Pod) => {
          ...
          if (matches) {
            matches = Object.entries(selector)
              .every(([key, value]) => { return itemLabels[key] === value});
          }
          return matches
        });
...
[bold]namespace:[/] {namespace}`;
  ...
  getControllerChartNode(controller: any, image: string,pods: K8sApi.Pod[]): any {
    ...
    if (controller.metadata?.labels?.heritage === "Helm" && controller.metadata?.labels?.release) {
      ...
      if (!this.helmData.find((item: any) => { return item.name == releaseName && item.namespace == controller.getNs() })) {
    ...
    return pods.map((pod) => {
      ...
      pod.getSecrets().forEach((secretName) => {
        ...
        if (secret) {
          ...
          if (!this.secretsData.find((item: any) => { return item.id === dataItem.id })) {
